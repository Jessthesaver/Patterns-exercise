...
import EventHandler from './command.js';
...
class Presenter {
    ...
    handleAddNote = () => {
        const addNoteCommand = new AddEvent(this.model, this.view);
        this.eventhandler.executeCommand(addNoteCommand);
    }
    ...
    handleDeleteNote = (args) => {
        if (args.class === 'note-remove') {
            ...
            if (doDelete) {
                const deleteNoteCommand = new DeleteEvent(this.model, this.view, this.oldNotes);
                this.eventhandler.executeCommand(deleteNoteCommand, args.id);
            }
        }
    }
    ...
    handleDragDrop = (newOrder) => {
        ...
        this.eventhandler.executeCommand(dragNoteCommand, newOrder);
    }

    handleUndo = () => {
        if (this.eventhandler.history.length > 0) {
            this.eventhandler.undo();
        } else if (this.eventhandler.history.length <= 0) {
            return;
        }
    }
}
